...
export default function ImportPage() {
  ...
  const doImport = async () => {
    ...
    const trades = rawData
      .map((row, idx) => {
        ...
        if (!out.date || !out.direction) {
          errorDetails.push(`Ligne ${idx + 1}: date ou direction manquante`);
          return null;
        }
        return out;
      })
    ...
    try {
      ...
      setResult({ success: 0, errors: trades.length, errorDetails: ["Erreur serveur"] });
      setStep(5);
    }
    setImporting(false);
  };
              ...
              >
                {importing ? (
                  ...
                  <>
                    ...
                    Importer {rawData.length} trades
                  </>
                )}
              </button>
            </div>
            ...
            <>
              ...
              {detection && (
                <p className="text-sm mt-4" style={{ color: "var(--text-secondary)" }}>
                  Source : {detection.platform.name}
                </p>
              )}
                ...
                >
                  ...
                  {result.errorDetails.length > 10 && (
                    <p style={{ color: "var(--text-muted)" }}>
                      ... et {result.errorDetails.length - 10} autres erreurs
                    </p>
                  )}
                </div>
              )}
