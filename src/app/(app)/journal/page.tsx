                        ...
                        >
                          ...
                          <td className="py-3 px-3 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              {/* Indicators for hidden data */}
                              {tradeTags.length > 0 && (
                                <span className="w-5 h-5 rounded flex items-center justify-center bg-cyan-500/15 text-cyan-400 text-[9px] font-bold" title={`Tags: ${tradeTags.join(", ")}`}>
                                  {tradeTags.length}
                                </span>
                              )}
                              {hasNotes && (
                                <span className="w-5 h-5 rounded flex items-center justify-center bg-amber-500/15 text-amber-400" title={trade.setup?.slice(0, 50)}>
                                  <FileText className="w-3 h-3" />
                                </span>
                              )}
                              {trade.screenshots && trade.screenshots.length > 0 && (
                                <span className="w-5 h-5 rounded flex items-center justify-center bg-purple-500/15 text-purple-400 text-[9px] font-bold" title={`${trade.screenshots.length} screenshot(s)`}>
